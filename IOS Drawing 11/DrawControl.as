package {
	...
	import flash.events.*;
	public class DrawControl extends MovieClip {
		var mainMC: MovieClip;
		var myParent: MovieClip;
		
		var drawControlLocOrigin: Point = new Point(750, 510);
		var drawControlLoc: Point = new Point(750, 510);
		var drawControlStartDrag = new Point(750, 510);
		...
		public function moveControlMove(event: MouseEvent): void {
			...
			if (moveControlMoveFlag == true) {
				...
				this.x = drawControlLoc.x;
				this.y = drawControlLoc.y;
				myParent.drawing.movePaper(new Point(drawControlLoc.x - drawControlStartDrag.x, drawControlLoc.y - drawControlStartDrag.y));
		...
		public function scaleControlMove(event: MouseEvent): void {
			if (scaleFlag == true){
				...
				myParent.drawing.imageData.scaleData(Math.pow(2,temp));
				myParent.drawing.updateCurrentPaper();
